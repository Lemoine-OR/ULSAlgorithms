...
public sealed class FedergruenTzurSolverTests
{
    [Fact]
    public void Solver_ImplementsCommonStrategyContract()
    {
        IUlsSolver solver = new FedergruenTzurSolver();

        Assert.Equal(UlsSolverKind.Exact, solver.Kind);
        Assert.Equal(
            "Federgruen-Tzur general O(n log n)",
            solver.Name);
    }

    [Fact]
    public void Solve_HandlesStrongSpeculativeMotives()
    {
        ...
        var result = new FedergruenTzurSolver().Solve(
            problem,
            token);
    ...
    [Fact]
    public void Solve_ZeroDemandPeriodCanBeOptimalProductionPeriod()
    {
        var problem = new UlsProblem(
            [0.0, 10.0],
            [1.0, 100.0],
    ...
    [Fact]
    public void PublishedWagelmansExample_AllFiveExactSolversAgree()
    {
        double[] demands =
        [
        ...
        var federgruenTzur =
            new FedergruenTzurSolver().Solve(problem, token);

        var wagelmansGeneral =
            new WagelmansGeneralSolver().Solve(problem, token);
        ...
        var linear =
            new WagnerWhitinSolver().Solve(problem, token);

        AssertClose(
            864.0,
            federgruenTzur.ObjectiveValue!.Value);

        AssertClose(
            federgruenTzur.ObjectiveValue.Value,
            wagelmansGeneral.ObjectiveValue!.Value);

        AssertClose(
            federgruenTzur.ObjectiveValue.Value,
            classical.ObjectiveValue!.Value);

        AssertClose(
            federgruenTzur.ObjectiveValue.Value,
            evans.ObjectiveValue!.Value);

        AssertClose(
            federgruenTzur.ObjectiveValue.Value,
            linear.ObjectiveValue!.Value);
    }

    [Fact]
    public void RandomGeneralInstances_MatchAllGeneralExactSolversAndOracle()
    {
        var token = TestContext.Current.CancellationToken;
        var random = new Random(19910801);

        var federgruenTzur = new FedergruenTzurSolver();
    ...
    {
        var token = TestContext.Current.CancellationToken;
        var random = new Random(19910809);

        var federgruenTzur = new FedergruenTzurSolver();
        var wagelmansLinear = new WagnerWhitinSolver();
    ...
    public void Solve_HonorsCancellation()
    {
        ...
        Assert.Throws<OperationCanceledException>(() =>
            new FedergruenTzurSolver().Solve(
                problem,
                cancellation.Token));
    }
