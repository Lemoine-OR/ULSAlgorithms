...
if (Test-Path -LiteralPath $readme) { $inputs.Add($readme) }
$src = Join-Path $root 'src'
if (Test-Path -LiteralPath $src) { $inputs.Add($src) }
$pages = Join-Path $PSScriptRoot 'pages'
if (Test-Path -LiteralPath $pages) { $inputs.Add($pages) }
...
$readmeValue = if (Test-Path -LiteralPath $readme) { Convert-ToDoxygenPath $readme } else { '' }
...
& $doxygen.Source $tempDoxyfile
if ($LASTEXITCODE -ne 0) {
    throw 'Doxygen documentation build failed.'
}
