    ...
    $files = @(
        Get-ChildItem -LiteralPath $releaseRoot -Recurse -File |
            Where-Object {
                ...
                $_.Extension -in @('.dll','.xml','.json','.deps.json','.runtimeconfig.json')
            }
    )

    $primaryDll = $files | Where-Object { $_.Name -eq "$projectName.dll" } | Select-Object -First 1
...
Compress-Archive -Path (Join-Path $validatedRoot '*') -DestinationPath $zipPath -CompressionLevel Optimal
